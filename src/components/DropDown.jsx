...
function DropDown() {
  ...
  return (
    <div className="relative w-24 flex flex-col items-center rounded-md">
      <button onClick={() => setIsOpen((prev) => !prev)} className="flex items-center justify-around border-transparent hover:border-secondary duration-400 hover:text-secondary">
      ...
      {isOpen && <div className="bg-primary-gradient absolute mt-10 max-w-full overflow-x-hidden">
        ...
        <li className='w-full flex justify-center items-center'>
          <NavLink to={'/games'} className={({ isActive }) => isActive ? 'text-secondary' : ''}>
            Games
          </NavLink>
        </li>
        </div>}
